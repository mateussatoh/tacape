...
<p align="center">
  ...
  <a href="./tests/run.sh"><img src="https://img.shields.io/badge/tests-48_passing-4c9a2a?style=flat" alt="Tests"></a>
...
<p align="center"><a href="./README.md">Português brasileiro</a></p>
...
tacape does this for the em dash. A `PreToolUse` hook on `Write`, `Edit` and `NotebookEdit` walks every string in the payload, and if a U+2014 is in there, the write is denied with an explanation before anything touches disk. Not a reminder. A wall.

The target is deliberately narrow: **U+2014 only, prose and UI copy only.** The complaint the guard answers is that machine-written text reads as machine-written, and the em dash is the strongest single tell. The scope follows that complaint and stops there. The en dash U+2013 is out, since it is correct in a numeric range and carries none of the tell. Non-prose files are skipped whole: a test fixture, a JSON manifest or a parser handling the character is doing legitimate work with it, and a guard aimed at how text reads has no business in a file nobody reads as text.

<details>
<summary><strong>Four non-obvious things about building this</strong></summary>
...
**All three layers must ban exactly the same thing.** The hook, the pre-commit and the CI invariant started with three slightly different definitions. That is three rules, not one, and the loosest one quietly teaches you to ignore the strictest. They share one target now, and the suite fails if they drift.
...
```bash
bash tests/run.sh                  # 48 assertions, no framework, no dependencies
